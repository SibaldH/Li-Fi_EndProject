...
void loop() {
  ...
  if (stringComplete) {
    inputString = convertProtocol(inputString);
    for(int i = 0; i <= inputString.length(); i++){
      transmitBit(inputString[i-1]);
    }
...
String convertProtocol(String _inputString) {
  String SOH = "00000001";
  String STX = "00000010";
  int _inputBody[] = {1,2,4,5};
  int _packages[] = {};
  int _length = 0;
  _length = sizeof(_inputBody) / sizeof(_inputBody[0]);
  String _lengthByte = byteToBinaryString((byte)_length);

  String _packageHead = SOH + _lengthByte;
  Serial.println(_packageHead);
  return _packageHead;
}

// ------------------  int convertDecToByte   ---------------------- //
